...
static int rtc_fetch_sysconfig_para(void)
{
	...
	if(SCIRPT_ITEM_VALUE_TYPE_INT != script_get_item("rtc_para", "rtc_used", &val)){
	...
	/* if(SCRIPT_PARSER_OK != script_parser_fetch("rtc_para", "rtc_used", (int*)&device_used, 1)){
	                printk("rtc: script_parser_fetch err. \n");
	                goto script_parser_fetch_err;
	} */
	...
	if(1 == device_used){
		...
		if(SCIRPT_ITEM_VALUE_TYPE_STR != script_get_item("rtc_para", "rtc_name", &val)){
		    pr_err("%s: script_parser_fetch err. \n", __func__);
			goto script_parser_fetch_err;
		}
		name = val.str;
		...
		if(SCIRPT_ITEM_VALUE_TYPE_INT != script_get_item("rtc_para", "rtc_twi_addr", &val)){
			pr_err("%s: script_parser_fetch err. \n", name);
			goto script_parser_fetch_err;
		}
		/* if(SCRIPT_PARSER_OK != script_parser_fetch("rtc_para", "rtc_twi_addr", (int*)&twi_addr, sizeof(twi_addr)/sizeof(__u32))){
			pr_err("%s: script_parser_fetch err. \n", name);
			goto script_parser_fetch_err;
		} */
		twi_addr = (__u32)val.val;
		u_i2c_addr.dirty_addr_buf[0] = twi_addr;
		u_i2c_addr.dirty_addr_buf[1] = I2C_CLIENT_END;

		/* if(SCRIPT_PARSER_OK != script_parser_fetch("rtc_para", "rtc_twi_id", (int*)&twi_id, 1)){
			pr_err("%s: script_parser_fetch err. \n", name);
			goto script_parser_fetch_err;
		} */
		if(SCIRPT_ITEM_VALUE_TYPE_INT != script_get_item("rtc_para", "rtc_twi_id", &val)){
			pr_err("%s: script_parser_fetch err. \n", name);
			goto script_parser_fetch_err;
		}
		twi_id = val.val;
...
static int pcf8563_set_datetime(struct i2c_client *client, struct rtc_time *tm)
{
    ...
    */
	...
	if(leap_year > 2073 || leap_year < 2010) {
		dev_err(&client->dev, "rtc only supports 63锛�2010锝�2073锛� years\n");
		return -EINVAL;
	}
